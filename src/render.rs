...
/// Attribution mark drawn in the bottom-right corner so anyone who sees a
/// diagram can trace it back to the tool. Default-on (the CLI `--no-mark` flag
/// suppresses it). The `generated by` prefix makes it read as provenance rather
/// than as part of the drawing. It is intentionally small and low-opacity, and
/// is *not* collision-avoided: on a tight canvas it may overlap content, in
/// which case the caller should disable it.
const MARK_TEXT: &str = "generated by cappyzawa/ponchi";
const MARK_FONT_SIZE: f32 = 11.0;
const MARK_COLOR: &str = "#000000";
const MARK_OPACITY: f32 = 0.35;
const MARK_PAD: f32 = 8.0;
...
/// and the attribution mark enabled (the product default).
pub fn render_scene(scene: &Scene) -> String {
    render_scene_with_font(scene, DEFAULT_FONT_FAMILY, true)
}
...
///
/// When `mark` is true, a small low-opacity attribution mark is drawn in the
/// bottom-right corner (see `MARK_TEXT`); callers pass `false` to suppress it.
///
...
pub fn render_scene_with_font(scene: &Scene, font_family: &str, mark: bool) -> String {
...
"#
    ...
    // Attribution mark (topmost, after everything else) when enabled.
    if mark {
        body.push_str(&attribution_mark_element(w, h, &font));
    }
...
/// Build the bottom-right attribution mark `<text>`. `font` must already be
/// XML-escaped (it is reused from the caller). The mark text is a constant but
/// is still XML-escaped for defense in depth. The anchor is pinned to the
/// bottom-right corner with a small padding; the baseline is kept within the
/// canvas height so it is never pushed past the bottom edge. On a canvas
/// narrower than the mark text it still overflows horizontally (the text cannot
/// shrink) — disable the mark there.
fn attribution_mark_element(w: u32, h: u32, font: &str) -> String {
    let (cw, ch) = (w as f32, h as f32);
    let x = if cw > MARK_PAD { cw - MARK_PAD } else { cw };
    let y = if ch >= MARK_FONT_SIZE + MARK_PAD {
        ch - MARK_PAD
    } else {
        ch.min(MARK_FONT_SIZE)
    };
    format!(
        r#"<text x="{x}" y="{y}" font-family="{font}" font-size="{MARK_FONT_SIZE}" fill="{MARK_COLOR}" fill-opacity="{MARK_OPACITY}" text-anchor="end">{}</text>
"#,
        xml_escape(MARK_TEXT)
    )
}
...
mod tests {
    ...
    /// Render with the attribution mark suppressed, for tests that assert on the
    /// exact set of drawn elements (the mark would otherwise add a `<text>`).
    fn render_unmarked(scene: &Scene) -> String {
        render_scene_with_font(scene, DEFAULT_FONT_FAMILY, false)
    }
    ...
    #[test]
    fn attribution_mark_present_by_default() {
        // The default API (render_scene) and the font variant both emit the mark.
        let s = scene_with(vec![node("a", 10.0, 10.0)], vec![]);
        for svg in [
            render_scene(&s),
            render_scene_with_font(&s, DEFAULT_FONT_FAMILY, true),
        ] {
    ...
    #[test]
    fn attribution_mark_absent_when_disabled() {
        let s = scene_with(vec![node("a", 10.0, 10.0)], vec![]);
        let svg = render_unmarked(&s);
        assert!(!svg.contains("cappyzawa/ponchi"), "{svg}");
    }

    #[test]
    fn attribution_mark_is_topmost() {
        // The mark must come after the node label so it is drawn on top.
        let s = scene_with(vec![node("a", 10.0, 10.0)], vec![]);
        let svg = render_scene(&s);
        let mark = svg.find("generated by cappyzawa/ponchi").expect("mark");
        let label = svg.find(">L<").expect("node label");
        assert!(label < mark, "mark must be drawn after the node label");
    }

    #[test]
    fn marked_render_is_deterministic() {
        let s = scene_with(vec![node("a", 10.0, 10.0)], vec![]);
        assert_eq!(render_scene(&s), render_scene(&s));
    }

    #[test]
    fn mark_baseline_stays_within_tiny_canvas() {
        // A canvas shorter than the mark font must not push the baseline past
        // the bottom edge (the text may still overflow horizontally — it cannot
        // shrink — but the baseline stays on-canvas).
        let el = attribution_mark_element(4, 5, DEFAULT_FONT_FAMILY);
        let y: f32 = attr(&el, "y");
        assert!(y <= 5.0, "baseline y={y} must be <= canvas height 5");
        assert!(y >= 0.0, "baseline y={y} must be >= 0");
    }

    /// Extract a numeric attribute value from a single SVG element string.
    fn attr(el: &str, name: &str) -> f32 {
        let key = format!("{name}=\"");
        let i = el.find(&key).expect("attr present") + key.len();
        let end = el[i..].find('"').unwrap();
        el[i..i + end].parse().unwrap()
    }
