...
struct State {
    ...
    font_family: String,
    /// Whether to draw the attribution mark. Held in shared state so every
    /// render — initial *and* each POST — uses the same value (otherwise the
    /// mark could appear on the initial render then vanish after a POST).
    mark: bool,
    token: String,
}
...
pub fn serve(
    ...
    let published = render_published(&raster, &initial, &font_family, mark, 1)?;
...
fn render_bytes(
    ...
    let svg = render_scene_with_font(scene, font_family, mark);
...
fn render_published(
    ...
    let (svg, png) = render_bytes(raster, scene, font_family, mark)?;
...
fn handle_post_scene(
    ...
    let (svg, png) = match render_bytes(&state.raster, &scene, &state.font_family, state.mark) {
        Ok(b) => b,
...
mod tests {
    ...
    #[test]
    fn render_bytes_honors_mark_flag() {
        use crate::scene::{Canvas, Scene};
        let raster = RasterContext::new(None, crate::render::DEFAULT_FONT_FAMILY);
        let scene = Scene {
        ...
        let font = crate::render::DEFAULT_FONT_FAMILY;
        let (svg_on, _) = render_bytes(&raster, &scene, font, true).unwrap();
        let (svg_off, _) = render_bytes(&raster, &scene, font, false).unwrap();
        assert!(svg_on.contains("generated by cappyzawa/ponchi"));
        assert!(!svg_off.contains("cappyzawa/ponchi"));
    }
}
