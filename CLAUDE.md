...
The bottom-right `generated by cappyzawa/ponchi` mark exists for a reason the code
can't tell you: it's a **discovery path** so a shared diagram can lead someone back
to the project. The `generated by` prefix is load-bearing — without it the bare slug
reads as if the *diagram* is about cappyzawa/ponchi, not as provenance.

Deliberate choices that look like bugs or cleanup opportunities:
- The mark is **not collision-avoided** and never grows the canvas. On a tight canvas
  it may overlap content — that is the user's cue to disable it, not something to
  engineer around (collision avoidance would tangle the mark into the layout pass).
- `serve` keeps the on/off flag in shared `State` so the initial render *and* every
  POST share one value — otherwise the mark would show once then vanish on the next
  POST. Keep it there if you touch the render path.
